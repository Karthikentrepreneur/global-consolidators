...
const FolderGallery = () => {
  ...
  const [folderInfo, setFolderInfo] = useState<GalleryFolder | null>(null);
  ...
  const fetchFolderInfo = async () => {
    try {
      ...
      console.error('Folder info fetch error:', error);
    }
  };
  ...
  const fetchFolderImages = async () => {
    setLoading(true);
    try {
      console.log('Fetching images for folder:', folderName, 'in country:', currentCountry);
      
      const { data, error } = await supabase
        .from('gallery')
        .select('id, title, description, image_url, created_at, folder_name')
        .eq('country', currentCountry)
  ...
  const handleBackToGallery = () => {
    ...
    const basePath = currentPath.replace(`/gallery/${folderName}`, '');
            ...
            <div className="text-center">
              <div className="flex items-center justify-center gap-3 mb-4">
                ...
                <h1 className="text-4xl font-bold text-gray-900">
                  {folderInfo?.display_name || folderName} - {countryNames[currentCountry]}
                </h1>
              </div>
          ...
          {!loading && images.length === 0 && (
            <div className="text-center py-16">
              ...
              <p className="text-gray-600">
                Images for this folder will appear here once they're uploaded.
              </p>
            </div>
          )}
